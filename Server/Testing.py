...
# Import necessary modules for QR code detection
import cv2
...
class Server:
    ...
    def __init__(self):
        ...
        # Initialize flag for QR code detection and action
    ...
    def sendvideo(self):
        ...
        while True:
            ...
            try:
                ...
                if qr_codes and not self.action_in_progress:
                    for qr_code in qr_codes:
                        qr_data = qr_code.data.decode('utf-8')
                        print(f"QR Code detected: {qr_data}")
                        # Start a new thread to perform the actions
                        action_thread = Thread(target=self.perform_qr_code_action)
                        action_thread.start()
    ...
    #When QR code is scanned, set hasMessage to true
    def perform_qr_code_action(self):
            ...
            while True:
                ...
                for oneCmd in cmdArray:
                    ...
                    if data is None:
                        ...
                        try:
                            # Remove the random condition and directly control the motors
                            data1=int(data[1])
                            data2=int(data[2])
                            data3=int(data[3])
                            data4=int(data[4])
    ...
    #spinOut tells the car to spinout, called when hit. 
    def spinOut(self):
        self.action_in_progress = True
        try:
            print("floor level spinning")
            self.PWM.setMotorModel(4095, 4095, -4095, -4095)  # Turn kart around
            self.led.colorWipe(self.led.strip, Color(255, 0, 0), 10)  # Set the color of all LEDs to red
            self.led.strip.show()  # Turn on the LEDs
            time.sleep(2)
            self.led.colorWipe(self.led.strip, Color(0, 0, 0), 10)  # Turn off the LEDs
            self.led.strip.show()
            self.PWM.setMotorModel(0, 0, 0, 0)  # Stop the motors
        except Exception as e:
            print(f"Error in perform_qr_code_action: {e}")
        finally:
            self.action_in_progress = False
...
if __name__ == '__main__':
    ...
    server.SendVideo = Thread(target=server.sendvideo)
    server.ReadData = Thread(target=server.readdata)
    server.SendVideo.start()
    server.ReadData.start()
